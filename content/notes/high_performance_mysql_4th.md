...
- 在事务中混合使用存储引擎  
  MySQL 不在服务器层面上管理事务。相反，底层存储引擎自己实现事务。这意味着你不能在一个事务中可靠地混合不同的引擎。如果你在一个事务中混合使用事务表和非事务表（例如，InnoDB 和 MyISAM 表），如果一切顺利，该事务将正常工作。然而，如果需要回滚，对非交易表的修改就不能被撤销。这就使数据库处于不一致的状态，可能很难恢复，并使事务的整个意义变得毫无意义。
