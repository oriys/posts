...
1. **数据传输延迟问题**
   ...
   - **延迟基准数据**：
     ...
     - **国内异地机房**：延迟 50ms 以内（北京-天津<10ms，北京-上海接近 30ms，北京-广州达 50ms）
...
2. **性能影响分析**
   ...
   - **可接受场景**：
     - 几次第三方 HTTP/RPC 服务调用：增加几毫秒延迟可接受
     - 几次数据库写入：增加几毫秒到十几毫秒可接受
   - **不可接受场景**：
     - 十几次到几十次缓存和数据库读取：增加几十到上百毫秒延迟不可接受

3. **数据访问策略挑战**

   - **跨机房直接读取**：B 机房应用直接读取 A 机房从库
   - **本地从库同步**：在 B 机房部署从库，跨机房同步主库数据
   - **共同问题**：都涉及跨机房数据传输和延迟问题
...
3. **具体实施方案**
   ...
   - **数据库部署**：
     ...
     - 主库部署在一个机房（如 A 机房）
     - A、B 两机房都写入 A 机房主库
     - A、B 机房各部署一个从库，通过主从复制同步数据
     - 查询请求读取本机房从库
     - 故障时通过主从切换将 B 机房从库提升为主库

   - **缓存部署**：

     - 两个机房都部署缓存
     - 查询优先读取本机房缓存
     - 缓存 miss 时穿透到本机房从库加载数据
     - 数据更新时更新双机房缓存保证一致性

   - **RPC 服务注册**：
     - 不同机房 RPC 服务注册不同服务组
     - RPC 客户端(Web 服务)只订阅同机房服务组
     - 实现 RPC 调用尽量在本机房内进行
...
3. **数据同步方案**
   ...
   - **存储系统主从复制**：
     ...
     - MySQL、Redis 等在一个机房部署主库，异地机房部署从库
     - 通过主从复制实现数据同步

   - **消息队列同步**：

     - 一个机房产生写入请求后写消息到消息队列
     - 另一个机房应用消费消息后执行业务逻辑写入存储

   - **组合使用策略**：
     - 消息方式同步缓存数据、HBase 数据等
     - 主从复制同步 MySQL、Redis 等数据

4. **用户分片与数据访问**

   - **用户分片依据**：按用户 ID、地域等属性进行分片
...
- **与 Agent 区别**：Agent 收集服务端信息，埋点从客户端角度描述组件和服务性能
- **实现方式**：
  - 面向切面编程（AOP）方式
  - 在资源客户端直接计算调用耗时、调用量、慢请求数

##### 埋点优化策略

- **问题**：缓存、数据库请求量高（单机每秒万次），直接发送会导致监控服务器不堪重负
- **解决方案**：先做汇总再发送
  - 每隔 10 秒汇总同一资源的请求量总和
  - 计算响应时间分位值、错误数等
  - 大大减少发往监控服务器的请求量
...
- **数据源**：Web 服务器和应用服务器访问日志
- **展示内容**：
  - 服务整体访问量
  - 响应时间情况
  - 错误数量
  - 带宽等信息
- **作用**：反映服务整体运行情况，帮助发现问题
...
- **展示内容**：
  - 被埋点资源的访问量
  - 响应时间情况
- **作用**：反映资源整体运行情况，定位具体问题资源或服务

#### 资源报表

- **数据源**：Agent 采集的资源运行情况数据
- **展示内容**：
  - 连接数变化情况
  - 缓存命中率
  - 资源具体运行指标
...
1. **配置项数量爆炸式增长**
   ...
   - 微服务架构下，每个服务都有独立的配置文件
   - 多环境部署（开发、测试、预发布、生产）成倍增加配置项
   - 第三方服务集成带来的配置复杂性
